...
export default class SingleArticlePreviewKupac extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            quantity: 1,
            loggedUser: localStorage.getItem("logged korisnik"),
            totalPrice: localStorage.getItem("sum"),
            visible: false,
        }
    }
    ...
    addToCart() {
        const data = {
            ...
            novaCena: this.props.article.cena.iznos * this.state.quantity,
        };

        api("korpe/proizvod/" + this.props.article.proizvodId + "/logovan", "post", data, "korisnik")
        .then(res => {
            if(res.status === "error") {
                return;
            }
            ...
            const event = new CustomEvent("cart.update.korisnik");
            ...
            <Col lg="4" md="6" sm="6" xs="12">
              <Card className="mb-3">
                ...
                <Card.Body  style={{ height: '240px'}}>
                  ...
                  <Link to = {`/prodavac-profile-log/${ this.props.article.prodavac.id }`} className="btn btn-primary w-100 btn-sm"
